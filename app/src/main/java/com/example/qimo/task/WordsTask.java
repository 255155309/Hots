...
import com.example.qimo.utils.Weibo;
...
public class WordsTask extends AsyncTask<String,Void,ArrayList<Words>> {
    CallBack back;

    public WordsTask(CallBack back) {
        this.back = back;
    }
    ...
    protected ArrayList<Words> doInBackground(String... strings) {
        Response response=Weibo.execute(strings[0]);
        try {
            String jsonData=response.body().string();
            ArrayList<Words> result=Weibo.parse(jsonData);
            return  result;
        } catch (IOException e) {
            e.printStackTrace();
        }
